...
RFMEMORY_SEGMENT FindFreePhysicalMemory(RFMEMORY_REGION_TABLE MemoryRegion, UINT64 NumBytes, UINT Align) {
    ...
    //             if(!BitMask) break;
    //             if((BitMask & 1) &&
    //             MemorySegment->HeapLength >= NumBytes
    //             ) {
    //                 __SpinLockSyncBitTestAndSet(&(UINT32*)MemorySegment, MEMORY_SEGMENT_SEMAPHORE);
    //                 if(((UINT64)MemorySegment->BlockAddress & (Align - 1)) + NumBytes > MemorySegment->HeapLength) {
    ...
    //                 __BitRelease(&(UINT32*)MemorySegment, MEMORY_SEGMENT_SEMAPHORE);
    //             }
    //         }
    //     }
