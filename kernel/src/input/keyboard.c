...
void handle_keyboard(uint8_t kb_code){
	...
	|| kb_code == 70 // scroll lock
		...
		if(UppercaseSub) UppercaseSub = 0;
		else UppercaseSub = 32;
	...
	if(kb_code & 0x80) {
		...
		GP_sf_put_char(QWERTY[kb_code] - UppercaseSub,0xFFFFFFFF,x,y);
...
// 	KeControlIrq((KEIRQ_ROUTINE)INTH_keyboard, 1, IRQ_DELIVERY_NORMAL, 0);
// }