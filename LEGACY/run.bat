...
qemu-system-x86_64 -no-shutdown -no-reboot -cpu max,xsave=on,avx=on,vmx=off,avx2=on,avx512f=on,avx512dq=on -m 2G -smp 1,cores=1,threads=1 -machine q35 -monitor stdio -drive file=os.img,format=raw -device qemu-xhci -device usb-kbd